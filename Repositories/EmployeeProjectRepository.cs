...
internal sealed class EmployeeProjectRepository(AppDbContext context) : GenericRepository<EmployeeProject>(context), IEmployeeProjectRepository
{
    ...
    public async Task<bool> EmployeeProjectExistsAsync(Guid userId, Guid projectId, CancellationToken cancellationToken) => await DbSet.AnyAsync(x => x.UserId == userId && x.ProjectId == projectId, cancellationToken).ConfigureAwait(false);

    public async Task AddEmployeeProjectAsync(EmployeeProject employeeProject, CancellationToken cancellationToken) => await DbSet.AddAsync(employeeProject, cancellationToken).ConfigureAwait(false);

    public async Task<EmployeeProject?> GetEmployeeProjectByIdAsync(Guid id, CancellationToken cancellationToken) => await DbSet.FirstOrDefaultAsync(x => x.Id == id, cancellationToken).ConfigureAwait(false);

    public void Remove(EmployeeProject employeeProject) => DbSet.Remove(employeeProject);
    ...
    public async Task<int> GetUserProjectsCountAsync(Guid userId) => await DbSet.AsNoTracking().Where(x => x.UserId == userId).CountAsync().ConfigureAwait(false);
    ...
    public async Task<IReadOnlyCollection<ProjectResponseDto>> GetUserProjectsByUserIdAsync(Guid userId, int pageNumber, int pageSize)
    {
        return await DbSet.AsNoTracking()
            ...
            .Select(x => new ProjectResponseDto
            {
                ...
                TotalUsers = DbSet.Count(ep => ep.ProjectId == x.ProjectId)
            })
            .ToListAsync()
            .ConfigureAwait(false);
    }
