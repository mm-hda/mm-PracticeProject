    ...
    : GenericRepository<EmployeeProject>(context), IEmployeeProjectRepository
{
    ...
    {
        var employeeProjects = await GetAsync(x => x.UserId == userId, cancellationToken: cancellationToken).ConfigureAwait(false);
        ...
        var projectUserCountDictionary = employeeProjects
            .GroupBy(x => x.ProjectId)
            .ToDictionary(x => x.Key, x => x.Count());

        var pagedProjects = employeeProjects.OrderByDescending(x => projectDictionary.TryGetValue(x.ProjectId, out var project)
                        ? project.StartDate
                        : DateTime.MinValue)
        ...
        return pagedProjects.Select(ep =>
            {
                ...
                projectUserCountDictionary.TryGetValue(ep.ProjectId, out var totalUsers);
